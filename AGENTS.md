...
- Source imported from historical research remains under `agent_closed_loop/` and `compile/`. Historical report builders and old version tests need omitted datasets/run caches. Use the portable release commands first.
...
7. Treat `confirmed_phase` as the monotone stage memory; `accepted_phase=0` means suspended by OOD. Probability argmax is not the execution state. There is no robot control in this package.
