"""Cache frozen Wan2.2 VAE observation features for online monitoring."""
...
from .visual_features import encode_episode_features, resolve_video_episodes
...
def main() -> None:
    ...
    # Release includes the same licensed VAE implementation used for training.
    if sieve_root is None:
        bundled = Path(__file__).resolve().parents[1]
        if (bundled / "third_party" / "wan22_vae").exists():
            sieve_root = bundled
    if sieve_root is None:
