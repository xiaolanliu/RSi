...
def detailed_step(stream,state,visual):
    ...
    energy,logits=stream.line(z);probs=logits.softmax(-1);mark('line_and_phase')
    ...
    logit,stream.head_cache=stream.head.step(zn,en,stream.head_cache);raw=float(logit.sigmoid()[0]);mark('single_risk_head')
    ...
    result=dict(frame_risk_score=raw,risk_score=risk,alarm=alarm,phase_probs=probs[0].cpu().numpy())
...
def main():
    ...
    data_manifest=json.loads((artifact_path(m['data'])/'manifest.json').read_text())
    ...
    s=OODV4Stream(checkpoint,a.device,episode_seed=episode_seed(r));full=[];outputs=[]
    ...
    result=dict(device=a.device,dimension=m['dimension'],backbone_epoch=m['backbone_epoch'],samples=n,warmup_frames=warmup,
